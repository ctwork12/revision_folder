...
dateSelection = filteredMDF['DataDate'].drop_duplicates().to_list()
dateSelectionoption = st.sidebar.selectbox("Date", dateSelection)

filteredMDF = filteredMDF.loc[(filteredMDF['DataDate'] == dateSelectionoption)]
filteredMDF = filteredMDF[['CompanyCode', 'DataDate', 'Horizon', 'MDFType', 'VariableID', 'MDFValue']]
...
mdfplot = alt.Chart(filteredMDF).mark_bar().encode(
    x='MDFValue',
    y="VariableID"
).properties(title = companyCodeoption + " on " + dateSelectionoption).interactive()
